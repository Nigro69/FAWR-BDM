        ...
        <div className="w-3/4">
                    ...
                    {codeData &&
                      codeData.map((data) => (
                        <div
                          key={data.dial_code}
                          onClick={() => {
                            setnumber(`${data.dial_code} ${" "} ${data.name} `);
                            setdrop(0);
                          }}
                          className="text-center p-2 text-sm bg-[#211F22] cursor-pointer text-gray-400"
                        >
          ...
          <div className="space-y-2 my-5">
            <div className="text-gray-400">
              ...
              {!pancardValidate && <label className="text-red-600 text-sm italic">Please enter valid pancard number</label>}
            </div>
            <input
              type="text"
              placeholder="Pan Card Number"
              onChange={(e)=>pancardValidation(e.target.value)}
              className={`focus:outline-0 uppercase w-3/4 text-sm bg-[#211F22] px-3 py-2 rounded-md border border-gray-400 text-gray-400 ${pancardValidate ? "border border-gray-400" : "border-2 border-red-600"}`}
            />
          </div>
          <div className="space-y-2 my-5">
            <div className="text-gray-400">
              ...
              <div className="w-full grid grid-cols-3 gap-2">
                {locations.map((location, index) => (
                  ...
                  >
                    ...
                    <IoMdCloseCircle onClick={() => deletelocation(index)} />
                  </div>
                ))}
              </div>
          ...
          <div className="flex gap-4 my-10">
            <div>
              <input type="checkbox" checked={agree} onChange={()=>setagree(!agree)} className="rounded-md" />
            </div>
          ...
          <div className="flex w-3/4 justify-between place-items-center my-5">
            ...
            <div className="grid place-items-center">
              <button disabled={!(agree && email && password && firstName && lastName && phoneNumber && pancardValidate)} onClick={handleSubmit} className="text-sm text-white bg-[#BC312E] px-6 py-2 rounded-md">
                Next
              </button>
            </div>
          </div>
        ...
        <div className="w-1/4 bg-black p-6">
          ...
          <div className="space-y-3">
            <div className="text-sm text-white">
              &#9675; The information provided here will be considered throughout the
                application process.
            </div>
            <div className="text-sm text-white">
              &#9675; A candidate who has been rejected by BDM in the past three months, cannot re-apply.
            </div>
            <div className="text-sm text-white">
              &#9675; For any queries or assistance, please write to us at support@bigbrosdigitalmedia.com.
            </div>
            <div className="text-sm text-white">
              &#9675; Please be careful while filling the profile information.
            </div>
            <div className="text-sm text-white">
              &#9675; Your resume would be considered for either our IT & IT-IS Operations or for our BPS operations, basis your choice of the Area of Interest specified here.
            </div>
          </div>
        </div>
      </div>}
      {emailverified && <div className="dark:bg-[#211F22] h-80 w-2/4 grid place-items-center space-y-3 rnd-shd rounded-md p-10">
        ...
        <div className="text-sm text-gray-400 text-center">
          Please check your email address and click on verification link to verify your account. After click on the button below to check your verification status.
        </div>
        <div className="w-full">
            {!verificationPending && <button onClick={check} className="bg-[#BC312E] text-white font-semibold font-sans text-center text-sm p-3 w-full rounded-md">Click here after you verify</button>}
            {verificationPending && <button className="bg-[#BC312E] text-white font-semibold font-sans text-center text-sm p-3 w-full rounded-md">Checking...</button>}
        </div>
      </div>}
    </div>
  );
